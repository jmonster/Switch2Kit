...
@main private enum ResultTests {
    static func main() {
        ...
        queue.sync {
            ...
            session.sendCommand(1, 5, Data(), completion: capture)
            ...
            session.sendCommand(1, 5, Data(count: 256), completion: capture)
            guard case .payloadTooLarge = failures.removeLast() else { fatalError("wrong payload failure") }
            radio.writeLimit = 8
            session.sendCommand(1, 5, Data([1]), completion: capture)
