...
GameCube's soft/strong firmware feedback remains available through `s2k_play_feedback`. The adapter does not advertise it as cancellable SDL game rumble, because firmware clips cannot implement SDL's arbitrary duration and immediate stop contract. It also does not advertise SDL motion sensors from unscaled raw counts. Raw motion remains accessible through the C binding; calibrated SI scaling and model-axis conversion are required before feeding SDL's sensor API.
...
A separate Swift fixture supplies fake physical controllers to the real event hub and C ABI. The C++ test uses actual SDL3 enumeration, mapping, events, duration expiry and detach behavior. It checks all four models, all 256 GameCube trigger values, button edges, overload, reconnect identity, inactivity and rumble lifetime. The fixture is not part of the adapter library. These tests do not claim physical Bluetooth or emulator gameplay validation.
