...
The existing `Integrations/Emulators/SDLHost.hpp` adds `loadMotionProfile(path)`, bounded explicit file loading outside SDL/host locks, as well as in-memory installation and removal. It does not choose a path or persist it implicitly. Hosts can save user-selected paths in their own settings and explicitly reload them. Controller input and physical identity survive removing calibration. This host API alone does not provide emulator UI selection or downstream integration; those pinned emulator changes remain a separate required step.
...
Current automated evidence is mathematical conversion, synthetic profiles and fake controller boundaries feeding the actual hub, C ABI and pinned SDL. A practical capture/solve tool, actual emulator profile UI and downstream reset processing, final native application builds, measured device profiles and physical acceptance must be tracked separately; a green adapter fixture is not gameplay qualification.
