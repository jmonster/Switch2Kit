...
Dolphin's existing event thread feeds a per-device 256-pair ring. Each emulated Wii Remote owns an independent read cursor and enablement request. Its actual cursor filter runs only for fresh pairs, with the mapped receive delta; the existing acceleration correction is scaled by elapsed time rather than by report count. The SDL-to-Dolphin frame (`-x, z, y` for both sensors) is applied once. MotionPlus/acceleration packet state may be read repeatedly, but packet reads never advance the physical cursor filter. Missing pairs, event stalls, consumer stalls, profile changes, input gating and retirement reset the real filter; a newly assigned reader skips another consumer's backlog. Host motion settings and physical assignments remain separate.
