...
> A successful binary build is **not redistribution permission**, a notarized release or physical-controller qualification. Resolve the [provenance blocker](provenance.md) before distributing externally.
...
CI performs the archive from outside the repository working directory, runs structural/interface checks and independent consumers, and preserves development artifacts and diagnostics. A ZIP checksum proves which bytes were inspected, not trust, licensing clearance, notarization or controller behavior.
