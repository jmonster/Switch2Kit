...
Start/stop are idempotent. Discovery windows are replaceable and generation-checked. Per-controller operations identify physical devices, not application player slots. Rumble intents coalesce in a bounded 64-ID inbox and carry the submitting session generation. LED/RSSI/companion operations use a 128-entry, batched queue with `operationQueueFull` backpressure. Delayed controls cannot operate on a replacement connection with the same device ID. Protocol commands inside a session remain serialized, bounded and reply-correlated.
