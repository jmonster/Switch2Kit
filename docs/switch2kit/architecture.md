...
The app owns its four-player policy, preferences, visualizers, optional outputs, permissions UI, and lifecycle. Library users do not inherit those dependencies or limits. NFC/audio capture and actuator-melody research tools are not shipped. Sensor configuration, motion/optical input, and supported rumble remain independent of that removed research subsystem.
