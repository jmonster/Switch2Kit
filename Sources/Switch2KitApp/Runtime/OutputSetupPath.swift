...
enum OutputSetupPath: String, CaseIterable, Identifiable {
    ...
    var detail: String {
        switch self {
        case .sdl:
            return "Build the SDL library for a compatible game. Install it in a separate game copy so the original application stays unchanged."
        case .retroarch:
    ...
    var guideURL: URL {
        URL(string: "https://github.com/jmonster/Switch2Kit/blob/main/" + guidePath)!
    }
}
