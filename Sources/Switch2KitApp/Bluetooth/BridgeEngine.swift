...
final class BridgeEngine: NSObject, ObservableObject, @unchecked Sendable {
    ...
    private let btQueue = DispatchQueue(label: "io.github.jmonster.switch2kit.outputs")
    ...
    private func experimental(_ action: ControllerToolAction, serial: String) {
        btQueue.async { [weak self] in
            guard let self, let session = self.sessions.values.first(where: { $0.serialNumber == serial }) else { return }
    ...
    private func receiveExperimental(_ event: ControllerToolEvent) {
        switch event {
