...
/// App-only NFC and headset tools.
...
struct ControllerToolTag: Sendable {
...
struct ControllerToolCapture: Sendable {
...
enum ControllerToolEvent: Sendable {
    ...
    case nfcTagRead(Switch2ControllerID, ControllerToolTag)
    /// An explicit capture finished or was stopped during retirement.
    case audioCaptureFinished(Switch2ControllerID, ControllerToolCapture)
    /// An experiment could not proceed.
    case failure(Switch2ControllerID, ControllerToolError)
}

/// App-owned controller tools using the manager's serial transport through package-only hooks.
final class ControllerTools: Sendable {
    private let manager: Switch2ControllerManager
    private let events: ControllerToolEventPipe
    ...
    init(manager: Switch2ControllerManager, on queue: DispatchQueue = .main,
        ...
        let events = ControllerToolEventPipe(queue: queue, handler: handler)
        self.events = events
    ...
    func setSensorProfile(_ profile: ControllerToolSensorProfile) {
...
enum ControllerToolSensorProfile: String, CaseIterable, Sendable {
...
final class ControllerToolEventPipe: Sendable {
