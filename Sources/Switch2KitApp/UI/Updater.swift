// Updater.swift
// Retained upstream updater; disabled by AppInfo.updatesEnabled in this application.
//
...
final class Updater: ObservableObject {
    ...
    /// Team checked by the installer. Automatic updates are disabled.
    ...
    func check(userInitiated: Bool) async {
        guard let url = feedURL else {
            if userInitiated { state = .failed(AppInfo.updatesEnabled ? "No update feed URL is configured." : "Updates are disabled in this application. Install reviewed builds manually.") }
            return
        }
        let prior = state
        state = .checking
    ...
    private nonisolated static func fetchAndVerify(
        ...
        let scratch = FileManager.default.temporaryDirectory
            .appendingPathComponent("switch2kit-update-\(entry.build)", isDirectory: true)
    ...
    private static func installAndRelaunch(newApp: URL) throws {
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("switch2kit-install-\(pid).sh")
        ...
        VStack(spacing: 14) {
            ...
            if updater.feedURL == nil {
                Text(AppInfo.updatesEnabled
                     ...
                     : "Install Switch2Kit updates manually.")
