...
enum AppInfo {
    ...
    static var sourceRevision: String {
        let revision = Bundle.main.infoDictionary?["Switch2KitSourceRevision"] as? String ?? "unrecorded"
        let dirty = Bundle.main.infoDictionary?["Switch2KitSourceDirty"] as? Bool ?? false
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            Text("Nintendo Switch 2 controllers on macOS")
                .foregroundStyle(.secondary)
            HStack {
                Link("Project", destination: URL(string: "https://github.com/jmonster/Switch2Kit")!)
                Link("Documentation", destination: URL(string: "https://github.com/jmonster/Switch2Kit/tree/main/docs")!)
            }
        }
        .padding(28)
        .frame(width: 380)
