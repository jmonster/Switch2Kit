...
enum AppInfo {
    ...
    static var sourceRevision: String {
        let revision = Bundle.main.infoDictionary?["S2KSourceRevision"] as? String ?? "unrecorded"
        let dirty = Bundle.main.infoDictionary?["S2KSourceDirty"] as? Bool ?? false
        return String(revision.prefix(12)) + (dirty ? " (modified)" : "")
    }

    /// Automatic updates are disabled; installations are managed by the user.
