...
final class LogPipeline: @unchecked Sendable {
    ...
    private let writerQueue = DispatchQueue(label: "io.github.jmonster.switch2kit.logging", qos: .utility)
    private let logger = Logger(subsystem: "io.github.jmonster.switch2kit", category: "bridge")
    ...
    convenience init() {
        let directory = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Logs/io.github.jmonster.switch2kit", isDirectory: true)
