...
env = dict(os.environ)
for key in ('SWITCH2KIT_EXPERIMENTAL_SENSORS', 'SWITCH2KIT_ENABLE_SENSOR_PROFILES'):
    env.pop(key, None)
profiles = []
for profile in ('compatibility', 'gamepad', 'motion', 'pointer'):
    trial = dict(env)
    if profile != 'compatibility':
        trial.update(SWITCH2KIT_EXPERIMENTAL_SENSORS=profile, SWITCH2KIT_ENABLE_SENSOR_PROFILES='1')
